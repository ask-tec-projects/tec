...
function getnerate_random_string_of_length(string_length: number): string {
    const any_non_alpha_char = /[^a-z]+/g;
    return Math.random().toString(string_length).replace(any_non_alpha_char, "");
}
...
describe("Password salting", () => {
    it("can salt and compare a password", () => {
        ...
        const comparison_result = PasswordSalter.validate_salted_password(random_password, salted_password);
